...
// it's a top level declaration, so it's an implicit any
let endTime: Date
...
// the initial value is a string, so we need to cast it to any first, then to Date
// since there is no intersection in Typescript between string and Date
